...
//       navigate("/home", { state: { userName: receivedUserName } }); // Pass userName back to Home
//     } catch (error) {
...
const CloseSlot = () => {
  ...
  const loadRazorpay = (amount) => {
    ...
    script.onload = () => {
      const options = {
        key: "rzp_test_FGHxXa9xdy1zY8", // Replace with your Razorpay Test Key ID
  ...
  const handleCloseSlot = async () => {
    try {
      const detailsResponse = await axios.post("https://localhost:7130/User/ClosetheSlot", {
        userName: receivedUserName,
      });
